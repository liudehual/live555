...
int MultiplexPs::gb28181AddFrame(char *buf,int len,int type)
{
	...
	switch(type){
		case IDR_FRAMES:{
			if((theError=gb28181AddIdrFrame(buf,len,psBuf,sizeof(psBuf),outLen))!=OS_NO_ERROR){
				return theError;
			}
			break;
		}		
...
int MultiplexPs::gb28181AddIdrFrame(char *frameBuf,int frameLen,char *psBuf,int psLen,int &outLen)
{
	int theError=OS_NO_ERROR;
	...
	gb28181AddPes(frameBuf,frameLen,
			&psBuf[PS_HDR_LEN+SYS_HDR_LEN+PSM_HDR_LEN],
			psLen-(PS_HDR_LEN+SYS_HDR_LEN+PSM_HDR_LEN),
			outLen);
